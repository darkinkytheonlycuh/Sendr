...
**Forever file sharing. No accounts. Up to 50 GB per file. 100% custom code — zero third-party services.**
...
- **50 GB uploads** via a custom chunked-transfer engine (parallel chunks, retry with backoff, pause/resume)
...
| --- | --- | --- |
| `SENDR_DATA_DIR` | `<project>/.sendr-data` | Where files + metadata live. Point at any big disk. |
| `SENDR_MAX_BYTES` | `53687091200` (50 GiB) | Max file size |
| `SENDR_CHUNK_SIZE` | `8388608` (8 MiB) | Upload chunk size (see Vercel note below) |
...
1. **Request bodies are capped at ~4.5 MB.** Set `SENDR_CHUNK_SIZE=4194304` (4 MiB) in
   ...
   from `/api/info`.
2. **Serverless filesystems are ephemeral.** `/tmp` is small and wiped — permanent
   ...
   (Docker command above) — same code, zero changes.
...
MIT licensed. Have fun sending. ✈️
