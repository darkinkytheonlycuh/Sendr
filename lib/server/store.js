...
async function blobPut(pathname, body) {
  if (blobAccess) {
    return put(pathname, body, { access: blobAccess, addRandomSuffix: false });
  }
  let lastErr;
  for (const access of ['private', 'public']) {
    try {
      const res = await put(pathname, body, { access, addRandomSuffix: false });
      blobAccess = access;
      return res;
    } catch (err) {
      lastErr = err;
...
function signedFetch(url, init) {
  return fetch(getDownloadUrl(url), init);
}
...
async function readMetaBlob(id) {
  ...
  const res = await signedFetch(entries[0].url, { cache: 'no-store' });
...
async function* blobChunkIterator(meta, start, end) {
  ...
  while (remaining > 0 && index < meta.chunkCount) {
    ...
    const res = await signedFetch(item.url, {
